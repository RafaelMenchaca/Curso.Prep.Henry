...
function operadoresLogicos(num1, num2, num3) {
  ...
  if (Math.sign(num1) === 0 || Math.sign(num2) === 0 || Math.sign(num3) === 0){
    ...
    var numTresAumentado = num3 + 1;
    return numTresAumentado; 
  }
  return false;
}
operadoresLogicos(10, 34, 59);
...
function tablaDelSeis(){
  ...
  for  (let i = 0; i <= 10; i++){
    let multiplosDeSeis =  i * 6;
    myArray.push(multiplosDeSeis);
  }
  return myArray;
}
...
function doWhile(numero) {
  ...
  let i = 0;
  let sumaDeCinco = numero;

  do {
    i = i + 1;
    sumaDeCinco = sumaDeCinco + 5;
  } while (i < 8);
  return sumaDeCinco;
}
doWhile(5);
